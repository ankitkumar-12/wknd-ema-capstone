 ...
 *
 * The magazine "Members Only" area is two compact teaser cards side by side
 * (Alaskan Adventure, Fly Fishing the Amazon), each an image plus a heading,
 * short description and a "Read More" action. We render them as a single
 * `columns` block (the `members-teaser` variant): one row whose cells are the
 * two teasers, so CSS lays them out as a responsive two-up grid — each cell
 * stacking image over content, matching the source.
 *
